...
async def test_generate_qa_pairs(mock_llm_client):
    ...
    with patch.object(
        generator.chunking_service, "semantic_aware_chunking", return_value=["Chunk 1", "Chunk 2"]
    ):
...
async def test_deduplication(mock_llm_client):
    ...
    with patch.object(
        generator.chunking_service, "semantic_aware_chunking", return_value=["Single Chunk"]
    ):
...
async def test_quality_filtering(mock_llm_client):
    ...
    with patch.object(
        generator.chunking_service, "semantic_aware_chunking", return_value=["Chunk"]
    ):
        samples = await generator.generate_qa_pairs("Text", num_questions_per_chunk=3)
        assert len(samples) == 1
