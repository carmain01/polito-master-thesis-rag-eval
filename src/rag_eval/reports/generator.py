...
class ReportGenerator:
    ...
    def to_console(self) -> None:
        ...
        for res in self.report.results:
            ...
            if res.metric_name not in worst_samples or res.score < worst_samples[res.metric_name][1]:
    ...
    def to_html(self, path: str | Path) -> None:
        ...
        for s_idx, res_dict in sample_results.items():
            ...
            table_rows += f'<td><button class="btn btn-sm btn-outline-secondary">+</button> {s_idx}</td>'
            ...
            for m, res in res_dict.items():
                safe_reason = res.reason.replace("<", "&lt;").replace(">", "&gt;") if res.reason else ""
    ...
    def to_html_comparative(
        ...
        for m in labels:
            ...
            delta_style = 'style="color: green;"' if delta < 0.05 else 'style="color: red; font-weight: bold;"'
