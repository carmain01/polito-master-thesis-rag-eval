...
def run_correlations(df: pd.DataFrame) -> dict:
    ...
    # PES vs move_score
    rho, p = stats.spearmanr(df["pes_score"], df["move_score"])
    results["pes_vs_move_score"] = {"rho": rho, "p_value": p, "n": len(df)}
...
def run_group_tests(df: pd.DataFrame) -> dict:
    ...
    results = {}
    groups = [
        df[df["teacher_move"] == move]["pes_score"].values
    ...
    if len(groups) >= 2:
        ...
        for i, move_i in enumerate(MOVE_ORDER):
            for j, move_j in enumerate(MOVE_ORDER):
                if j <= i:
                    continue
                g_i = df[df["teacher_move"] == move_i]["pes_score"].values
                g_j = df[df["teacher_move"] == move_j]["pes_score"].values
...
def plot_boxplot(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    sns.boxplot(
        data=df, x="teacher_move", y="pes_score",
    ...
    # Add mean markers
    means = df.groupby("teacher_move")["pes_score"].mean()
...
def plot_heatmap(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    cols = [c for c in ["pes_score", "M1_uptake", "M2_linguistic", "M3_disclosure", "move_score"] if c in df.columns]
...
def main() -> None:
    ...
    # ── Statistical Tests ──
    print("\n" + "═" * 80)
    print("  STATISTICAL VALIDATION: PES vs MathDial Teacher Moves")
    print("═" * 80)
    ...
    plot_scatter_pes_vs_move(df, output_dir)

    # ── Summary Tables ──
    print("\n" + "=" * 80)
    print("  PES by Teacher Move (ordered by pedagogical quality)")
    print("=" * 80)
    summary = df.groupby("teacher_move")["pes_score"].agg(["count", "mean", "median", "std"])
    ...
    for move in MOVE_ORDER:
        ...
        print(f"  {move:10s}  M1={sub['M1_uptake'].mean():.3f}  M2={sub['M2_linguistic'].mean():.3f}  M3={sub['M3_disclosure'].mean():.3f}  PES={sub['pes_score'].mean():.3f}")
